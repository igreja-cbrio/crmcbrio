...
Atualizado em: 2026-05-26 (v11.0) — **Agente Executor Financeiro** (primeiro agente "ativo" do sistema). Novo `agent_type='agent_executor_financeiro'` no framework. Diferente dos auditores: usa **tool use** do Claude API (loop agentic via `AgentService.runWithTools`) com 6 tools de leitura e 3 de escrita. Tools de escrita NÃO tocam no banco — enfileiram em `agent_queue` (status `pending`) com `action_type` + `payload` + `reasoning`. Gestor aprova/rejeita em uma nova seção "Fila de Aprovação" no `/assistente-ia`. Aprovar = chama `applyQueueAction()` que executa de fato (categorize_transaction, mark_payable_paid, reimbursement_decision) e marca a row como `applied` ou `failed`. Migration `041_agent_queue_executor.sql` adiciona colunas `action_type/action_label/reasoning/applied_at/apply_error` + status enum com `applied`/`failed`. **Aplicar manualmente no Supabase SQL Editor.** Arquivos: [financeiroExecutor.js](backend/agents/financeiroExecutor.js), [financeiroTools.js](backend/agents/tools/financeiroTools.js), [financeiroApply.js](backend/agents/tools/financeiroApply.js), [agentService.js:147](backend/services/agentService.js) (runWithTools). Próximos módulos seguem o mesmo padrão: tool registry + apply handlers + register no `/run` switch.
Atualizado em: 2026-05-13 (v10.12) — Logs diagnósticos no módulo de Eventos pra parar de chutar quando aparece 500: (1) sentinel `[CBRio Eventos v10.12] frontend carregado` no console na primeira render — se não aparecer, deploy é antigo; (2) `toggleEventStatus` loga start/API ok/API throw/refetch result com freshStatus + sucessoReal + willSuppress. Deve ser removido em uma futura release; é diagnóstico temporário.
Atualizado em: 2026-05-13 (v10.11) — Frontend `Eventos.jsx`: (1) finalizar ocorrência agora chama `loadEvents()` + `loadDash()` além de `refreshDetail()`/`loadOccurrence()` — antes a lista ficava com a data antiga em vermelho até F5; (2) `toggleEventStatus` (finalizar/reabrir evento principal) faz refetch pós-API e **suprime mensagem de erro lateral se o evento de fato mudou de status no banco** — evita UX confusa "Erro X" + evento finalizado. Erro real (UPDATE primário falhando) continua aparecendo porque o refetch detecta status inalterado.
