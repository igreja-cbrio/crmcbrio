...
export default function AttachmentButton({ eventId, taskId, taskType = 'event', phaseName, area, onAttachmentChange }) {
    ...
    <div style={{ position: 'relative', display: 'inline-block' }}>
      ...
      {open && (
        <div ref={panelRef} onClick={e => e.stopPropagation()} style={{
          ...
          {/* Error */}
          {error && (
            <div style={{ padding: '8px 16px', background: '#fee2e2', color: C.red, fontSize: 11 }}>
              {error}
          ...
          <div style={{ maxHeight: 200, overflowY: 'auto' }}>
            {items.length === 0 ? (
              ...
              <div key={a.id} style={{ padding: '8px 16px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ padding: '12px 16px', borderTop: `1px solid ${C.border}`, background: C.bg }}>
            ...
            >
              {uploading ? (
                ...
                <><Upload style={{ width: 14, height: 14 }} /> Anexar arquivo</>
              )}
            </button>
